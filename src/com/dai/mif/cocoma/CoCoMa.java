...
public class CoCoMa {
	...
	private static String productRevision = "Build: @@Cognos 11/2022-08-10_1527/662@@ ";

	/** Mail Address displayed in CoCoMa Help text **/
	/* maybe overwritten by commandline argument --mailto */
	public static String COCOMA_MAILTO_HELPTEXT = "ralf.roeber@amvara.de";

	/** UID used as key for password encryption */
	private static final String COCOMA_UID = "07369e30-1175-11df-8a39-0800200c9a66";

	/**
	 * Error code denoting that an error occurred but the application could continue
	 * its work.
	 */
	...
	/**
	 * The default file name for the configuration - can be overridden via command
	 * line.
	 */
	...
	/*
	 * If using --crypt on commandline, this variable stores the cleartext password
	 * as next argument from commandline
	 */
	...
	private static boolean dumpSecurityAndAccounts = false;
	private static boolean dumpDispatchers = false;

	/**
	 * Convenience method to extract the acutal command from a command line
	 * argument. This program currently support thre ways of defining a command line
	 * argument: prefixed by --, - or /.
	 * 
	 * @param arg The argument from which the actual command is to be extracted
	 * @return The extracted command, or the original argument value, if the command
	...
	/**
	 * Construct a string showing help information for this program. It shows all
	 * recognized command line arguments and gives a short description of the
	 * features available via command line.
	 * 
	...
	private static String getHelpString() {
		...
		helpString += "--check" + lf + "checks the configuration file for syntax errors and prints out the result" + lf
				+ lf;
		helpString += "--dispatcherinfo" + lf + "reads config file and checks dispatcher for information" + lf + lf;
		helpString += "--console" + lf + "prints the log messages to console as well as to the config file" + lf + lf;
		helpString += "--phasebasic" + lf
				+ "enters the basic configuration phase to set up general parameters for dispatchers, "
				+ "Cognos 8 security etc." + lf + lf;
		helpString += "--phasedeployment" + lf + "performs deployments for the configured system" + lf + lf;
		helpString += "--phasecontent" + lf
				+ "enters content configuration phase where security settings are applied to the "
				+ "various content objects" + lf + lf;
		helpString += "--crypt <password in cleartext>" + lf
		...
		helpString += "--fixPersonalFolderPermissions" + lf + "Updates account with incorrect account policies camids."
				+ lf
				+ "This is a very special option, where User CAMIDs change because of Cognos Namespace Configuration."
				+ lf + lf;
		helpString += "--force" + lf
				+ "if used with --fixPersonalFolderPermissions it will update all the users even if the account has correct policies."
				+ lf + lf;
		helpString += "--runSearchPath <searchPath>" + lf
				+ "Returns all the objects found in searchPath and an exception if searchPath is invalid" + lf + lf;
		...
		helpString += "Almost all options may be combined, e.g. java -jar CoCoMa.jar --dispatcherinfo --config COCOMA_CONFIG_LOCAL.xml --donotaskforbackup"
				+ lf + "See CoCoMa documentation for details on howto configure config.xml files." + lf
				+ "---------- end of help information  --------------------------------" + "" + "" + lf + lf;
		return helpString;
	}
	 ...
	 * 
	 * @param args Command line arguments passed when program was started.
	 */
	public static void main(String[] args) {
		...
		if (infoMode) {
			...
			if (CoCoMa.isDumpSecurityAndAccounts() == true) {
				C8Utility c8util = new C8Utility(c8Access);
				c8util.dumpSecuriyAndAccounts();
				log.info("done.");
				System.exit(0);
			}
			...
			if (REPORT_VERSIONS) {
				C8Utility c8util = new C8Utility(c8Access);
				c8util.reportVersions(cocoma.configuration.getReportVersions());
				log.info("done");
			}

			if (RUN_SEARCHPATH) {
				C8Utility c8util = new C8Utility(c8Access);
				c8util.fetchObjectsWithQueryOptions(SEARCHPATH_TO_RUN, c8util.setPropEnum(), new Sort[] {},
						c8util.setQORefProps());
				log.info("done");
			}
	...
	/**
	 * Construct a string containing information about the product (name, version,
	 * SVN revision etc).
	 * 
	...
	private static boolean REPORT_VERSIONS = false;
	private static boolean RUN_SEARCHPATH = false;
	private static String SEARCHPATH_TO_RUN;

	/**
	 * Start the actual work by applying the configured data to the C8 system to
	 * which the connection is currently established.
	 * 
	 * @param c8Access C8Access instance that provides access to the currently
	 *                 connected C8 system
	 */
	private void process(C8Access c8Access) {

		// create full content store backup ... always usefull before any changes
		// destroy your environment.
		if (this.configuration.getBackupData().isEnabled()) {
			log.info("----------------------------");
			log.info("Creating Contentstore Backup");
			C8ExportDeployment export = new C8ExportDeployment(c8Access, this.configuration.getBackupData());
			export.createExport();
			log.info("done");
			log.info("----------------------------");
		}
		...
		if (phaseAdvancedDispatcherSetting) {
			...
			if (dispatchers.size() != 0) {
				...
				log.info("There are " + dispatchers.size() + " dispatchers to be configured.");
				...
				for (DispatcherData dispatcherData : dispatchers) {
					...
					if (dispatcherData.getAdvancedParameters().length > 0) {
						log.info("Applying " + dispatcherData.getAdvancedParameters().length
		...
		try {
			...
			if (phaseContentConfiguration) {
				log.info("Performing content configuration");
			...
			if (phaseDeployment && !interactiveMode && !checkConfigMode) {
				...
				MailserverData mailServer = this.configuration.getMailserverData();

				for (DeploymentData deployment : this.configuration.getDeployments()) {
					if (deployment.getMailRecipient().length() > 0) {
						log.debug("Mail recipient: " + deployment.getMailRecipient());

						String subject = mailServer.fillPlaceholders(deployment.getMailSubject(), deployment,
								productVersion);
						log.debug("Mail subject: " + subject);

						String mailtext = mailServer.fillPlaceholders(deployment.getMailText(), deployment,
								productVersion);

						log.debug("Sending mail(s) ... ");
						mailServer.sendMail(subject, mailtext, deployment.getMailSender(),
	...
	/**
	 * @param processAdvancedDispatcherSetting (C8Access c8Access)
	 */
	...
	private void processDeployments(C8Access c8Access) throws CoCoMaC8Exception {
		...
		if (!deployments.isEmpty()) {
			if (deployments.size() == 1) {
				...
				log.info("There are " + deployments.size() + " deployment to be executed.");
			}
			...
			if (this.configuration.getBackupData().isEnabled() == false && phaseBackupDonotAsk == false) {
				...
				doDeployment = showDeploymentConfirmationPrompt(
						lf + "Are you sure, you want to execute the deployments?" + lf
								+ "You should only do so, if you have created a backup of the content store before."
								+ lf + "Disable this question: use --donotaskforbackup on commandline " + lf
								+ "Or configure backup section in config.xml, setting backup to true." + lf
								+ "If you really want to execute the deployments, please answer: " + answer + lf + lf
								+ "Your answer: ",
						answer);
			...
			if (doDeployment) {
				...
				for (DeploymentData depData : deployments) {
					...
					log.info("------------------------------------");
					log.info("Processing deployment '" + depData.getName() + "'" + "( " + depl_number + " of "
							+ deployments.size() + ")");
					log.info("------------------------------------");
					// Check status of last deployment
					if (depData.getStatus() == DeploymentData.DEPLOYMENT_STATUS_ERROR) {
						log.info("Skipping deployment \"" + depData.getName()
					...
					if (deleteItems.size() > 0) {
						log.info("Found " + deleteItems.size()
					...
					// Execute Deployment
					log.debug("Executing deployment '" + depData.getName() + "'" + "( " + depl_number + " of "
							+ deployments.size() + ")");
					C8Deployment deployment = new C8Deployment(depData, c8Access);
					log.info("Deployment      : " + depData.getName());
					log.info("Deploymentfolder: " + depData.getDeploymentFolder());
					log.info("Archive         : " + depData.getArchive());

					deployment.execute();
					log.info("Done executing deployment '" + depData.getName() + "'" + "( " + depl_number + " of "
							+ deployments.size() + ")");
					...
					if (depData.getRunCureJar().equalsIgnoreCase("true")) {
						...
						try {
							cure.updateQueryReport(c8Access, depData.getRunCureJar_searchPath(),
						...
						log.info(
								"Set options in config.xml to execute cure.jar and set all reports to use 'modellLast()' version.");
					}
				}
			} else {
			...
			log.debug("No deployment-tag found in XML config-file. Nothing to deploy.");
		}
	}
	...
	/**
	 * Prepare the actual work by reading the configuration from the config file and
	 * establishing the connection to the c8 server.
	 * 
	...
	private C8Access prepare() {
		...
		if (configFile.exists()) {
			if (configFile.canRead()) {
				...
				try {
					log.info("Reading configuration from file " + configFileName);
					this.configuration = new CoCoMaConfiguration();

					this.configuration.read(configFile);
					log.info("Config file version: " + this.configuration.getVersion());

					if (checkConfigMode) {
						String configCheckResult = "Configuration in " + configFileName + " is ok.";
					...
					if ((!interactiveMode) && (!checkConfigMode)) {
						...
						access = new C8Access(serverData.getDispatcherURL(), serverData.getNameSpace(),
								serverData.getUserName(), serverData.getPassword());

						log.info("Connecting to server " + serverData.getDispatcherURL());
						log.info("Cognos Version: " + serverData.getServerVersion());
						log.info("With user " + serverData.getUserName());

						boolean connected = false;
						if (serverData.getServerVersion().equals("10")) {
							try {
								connected = access.ConnectToCognosServer(serverData.getDispatcherURL());
							} catch (Exception e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
							...
							log.info(
									"No methods to connect to server other than 10. Set <version>10</version> in <server>-section");
							System.exit(2);
						}

						if (connected) {
							log.info("Successfully connected to the server");
							log.debug("Connected to " + serverData.getDispatcherURL() + " with user "
					...
					access = null;
					String msg = "Error reading the configuration: " + e.getMessage();
					CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_CRTICAL_ERROR, msg);
					log.error(msg);
					log.error(e.getMessage(), e);

					if (checkConfigMode) {
						String configCheckResult = "Configuration in " + configFileName + " is invalid: "
								+ e.getMessage();
						System.out.println(configCheckResult);
					}
				} catch (ConfigException e) {
					access = null;
					String msg = "Invalid configuration data: " + e.getMessage();
					CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_CRTICAL_ERROR, msg);
					log.error(msg);
					log.error(e.getMessage(), e);
				...
				access = null;
				String msg = "The config file exists but cannot be read. (" + configFile.getAbsolutePath() + ")";
				CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_CRTICAL_ERROR, msg);
				log.error(msg);
			}
		} else {
			access = null;
			String msg = "The config file does not exist. (" + configFile.getAbsolutePath() + ")";
			CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_CRTICAL_ERROR, msg);
			log.error(msg);
		}
	 ...
	 */
	private void processContentConfiguration(C8Access c8Access) throws CoCoMaC8Exception {
		...
		RestrictedContentData limitedContentData = this.configuration.getRestrictedContentData();
		if (limitedContentData.getRestrictionsDefined()) {
			log.info("Setting up content restrictions");
			C8ContentRestriction visibility = new C8ContentRestriction(limitedContentData, c8Access);
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void processBasicConfiguration(C8Access c8Access) throws CoCoMaC8Exception {
		...
		log.debug("There are " + dataSources.size() + " data sources to be processed.");
		...
		log.debug("There are " + dispatchers.size() + " dispatchers to be configured.");
		...
		if (secData.getSecurityTagFoundInXMLConfig()) {
			...
			List<CapabilityData> capabilities = this.configuration.getCapabilities();
	...
	private static void readCommandlineArguments(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (arg.startsWith("--") || arg.startsWith("-") || arg.startsWith("/")) {
				...
				if (command.equalsIgnoreCase("phasedeployment") || command.equals("phasedeploy")) {
				...
				if (command.equalsIgnoreCase("dumpSecurityAndAccounts"))
					setDumpSecurityAndAccounts(true);
				...
				if (command.equalsIgnoreCase("runSearchPath")) {
					RUN_SEARCHPATH = true;
					SEARCHPATH_TO_RUN = args[i + 1];
				}
		...
		if (!phaseBasicConfiguration && !phaseDeployment && !phaseContentConfiguration && crypt.length() == 0
				&& !infoMode) {
			phaseBasicConfiguration = true;
			phaseDeployment = true;
			phaseContentConfiguration = true;
	...
	/**
	 * Show a prompt on STDOUT and read the user's input from STDIN. The Input is
	 * compared with the given confirmation string, if the strings match, TRUE is
	 * returned, false if the strings dont match.
	 * 
	 * @param prompt       The Prompt to be shown.
	 * @param confirmation The answer that is expected as confirmation.
	 * 
	 * @return TRUE if the prompt was confirmed, FALSE if not
	 */
	private boolean showDeploymentConfirmationPrompt(String prompt, String confirmation) {
	 ...
	 * contains the product name and version as well as a short overview over the
	...
	/**
	 * Set the static error code. If the given code is > 0 and the currently saved
	 * static error code still is 0, the given code is applied. Otherwise the
	 * previously set code remains. The static error code is used to define the exit
	 * code the application shall return when ending.
	 * 
	...
	public static boolean isDumpSecurityAndAccounts() {
		return dumpSecurityAndAccounts;
	}

	public static void setDumpSecurityAndAccounts(boolean dumpSecurityAndAccounts) {
		CoCoMa.dumpSecurityAndAccounts = dumpSecurityAndAccounts;
	}

	public static void setREPORT_VERSIONS(boolean REPORT_VERSIONS) {
		CoCoMa.REPORT_VERSIONS = REPORT_VERSIONS;
	}
}
