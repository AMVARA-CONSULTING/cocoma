...
public class CoCoMa {
    ...
    private static String productRevision = " Build: @@Cognos 10.2.1/2015-03-15_1029/24@@ ";
                ...
                if (interactiveMode) {
                    ...
                    if (c8Access != null) {
                        if (c8Access.isConnected()) {
                            ...
                            if (cocoma.configuration.getBackupData()
                                    .isEnabled()) {
                            	log.info("Creating Contentstore Backup");
                                C8ExportDeployment export = new C8ExportDeployment(
                                        c8Access,
                                        cocoma.configuration.getBackupData());
                                export.createExport();
	...
	private void process(C8Access c8Access) {
	    ...
	    try {
	        ...
	        if (phaseBasicConfiguration) {
	            log.info("Perfoming basic configuration.");
	...
	private void processDeployments(C8Access c8Access) throws CoCoMaC8Exception {
	    ...
	    if (!deployments.isEmpty()) {
	        ...
	        if (this.configuration.getBackupData()
	                                .isEnabled() == false ) {
	        ...
	        if (doDeployment) {
	        	...
	        	for (DeploymentData depData : deployments) {
	                ...
	                log.info("Processing deployment '" + depData.getName()
	                        + "'" + "( "+ depl_number +" of "+deployments.size()+")");
	                log.info("------------------------------------");
	            	...
	            	if (deleteItems.size()>0) {
	            		log.info("Found "+deleteItems.size()+" configured items to delete. Looping over items to delete before deployment.");
	...
	private void processBasicConfiguration(C8Access c8Access)
	    ...
	    // --- set up the roles defined in the configuration
	
	    log.info("Setting up Cognos security");
