...
public class RoleSecurity extends AbstractSecurityObject {
    ...
    public void readConfig(XMLConfiguration conf, String configKey) {
        ...
        this.keepExistingMembers = conf.getBoolean(configKey + ".keepExisting",
                false);
        ...
        String[] removedRoles = conf.getStringArray(configKey
                + ".removeMembers.role");
        ...
        String[] removedGroups = conf.getStringArray(configKey
                + ".removeMembers.group");
        ...
        String[] removedUsers = conf.getStringArray(configKey
                + ".removeMembers.user");
    ...
    public void assignMembers(List<AbstractSecurityObject> securityObjects,
        ...
        for (AbstractSecurityObject aso : securityObjects) {
            ...
            if (this.roleMembers.contains(defaultName)
                ...
                for (BaseClass member : members) {
                    ...
                    String asoPath = aso.getC8Object().getSearchPath()
                            .getValue();
        ...
        for (BaseClass member : members) {
            ...
            try {
                ...
                log.error("Member with Username or Defaultname 'null' found !!!");
                log.error("You should - review members of " + query + " !!!");
                // e.printStackTrace();
                String msg = "Member with Username or Defaultname 'null' found in "
                        + query
                        + ". Clean up by hand or use UMA function \"Inkonsistenzen loeschen\".";
                CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_MINOR_ERROR, msg);
            }
            if (((member instanceof Role) && this.removedRoleMembers
                    .contains(memberName))
                    || ((member instanceof Group) && this.removedGroupMembers
                            .contains(memberName))
                    || ((member instanceof Account) && this.removedUserMembers
                            .contains(memberName))) {
    ...
    public void dumpMembers() {
        log.debug("Members of role " + getName() + " (" + roleMembers.size()
