...
public class CognosAccountInformation {
	...
	public String setLanguageToAccount(String account, String language) {
		...
		String myAccountSearchpath = "CAMID('" + c8Access.getNameSpaceName() + "')//*[@userName='"
				+ c8Access.getUsername() + "'][@objectClass='account']";
		...
		// Query Objects
		BaseClass[] bcResults = ch.fetchObjectsWithQueryOptions(myAccountSearchpath, ch.setPropEnum(), new Sort[] {},
		...
		try {
			...
			if (myAccount.getContentLocale().getValue() != null && myAccount.getProductLocale().getValue() != null
			...
			try {
				BaseClass[] updatedItems = c8Access.getCmService().update(new BaseClass[] { updatedAccount },
						new UpdateOptions());
			...
			logger.info("Found error on searching for usr with CAMID ... will not set default language to english");
			e.printStackTrace();
		}
		return "";
	}
