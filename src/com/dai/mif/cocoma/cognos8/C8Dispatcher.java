...
import com.cognos.developer.schemas.bibus._3.Dispatcher_Service;
...
public class C8Dispatcher {
	...
	public void setAdvancedDispatcherSettings() throws RemoteException {
		...
		PropEnum props[] = new PropEnum[] { PropEnum.defaultName,
				PropEnum.searchPath, PropEnum.advancedSettings };
		...
		String[][] advancedDispatcherSettings = this.dispatcherData
		...
		for (int i = 0; i < advancedDispatcherSettings.length; i++) {
			...
			log.info("Value: " + advancedDispatcherSettings[i][1]); // do
																	// something
			setting += "<setting name=\"" + advancedDispatcherSettings[i][0]
					+ "\">" + advancedDispatcherSettings[i][1] + "</setting>";
		}
		setting += "</settings>";
	...
	public void updateDispatcherParameters() throws CoCoMaC8Exception {
		...
		try {
			...
			if (this.dispatcherData.isGlobaleDispatcher()) {
				...
				queryPath = "/configuration//dispatcher[@defaultName=\""
			...
			BaseClass[] dispatchers = cmService.query(
					new SearchPathMultipleObject(queryPath), props, sort,
					queryOptions);

			if (dispatchers.length == 0) {
				throw new CoCoMaC8Exception(
						"No dispatcher found with the name "
								+ this.dispatcherData.getDispatcherName());

			} else if (dispatchers.length > 1) {
				throw new CoCoMaC8Exception("Dispatcher name "
						+ this.dispatcherData.getDispatcherName()
						+ " is ambiguous. Returned " + dispatchers.length
						+ " results.");
			} else {
				...
				if (this.dispatcherData.isGlobaleDispatcher()) {
					...
					log.debug("Found global dispatcher at "
					...
					setInteractiveReportingProcessLimit(disp,
							this.dispatcherData);
					...
					log.debug("Updating dispatcher "
							+ disp.getDefaultName().getValue() + " at "
							+ disp.getSearchPath().getValue());
					...
					log.debug("Configuration saved for dispatcher "
							+ disp.getDefaultName().getValue());
				}
			...
			throw new CoCoMaC8Exception(
					"Error accessing configuration for dispatcher "
							+ this.dispatcherData.getDispatcherName()
							+ ". C8 Server returned: " + e.getMessage());
		}
	 ...
	 */
	private void setAuditLevels(Configuration conf, DispatcherData dispData)
			throws CoCoMaC8Exception {
		...
		AuditLevelEnum cmsAuditLevel = AuditLevelEnum.fromString(dispData
				.getCmsAuditLevel());
		AuditLevelEnumProp cmsAuditLevelProp = new AuditLevelEnumProp();
		cmsAuditLevelProp.setValue(cmsAuditLevel);

		log.debug("Audit level content manager service: "
				+ dispData.getCmsAuditLevel());

		// --- report data service

		AuditLevelEnum rdsAuditLevel = AuditLevelEnum.fromString(dispData
				.getRdsAuditLevel());
		AuditLevelEnumProp rdsAuditLevelProp = new AuditLevelEnumProp();
		rdsAuditLevelProp.setValue(rdsAuditLevel);

		log.debug("Audit level report data service: "
				+ dispData.getRdsAuditLevel());

		// --- event management service

		AuditLevelEnum emsAuditLevel = AuditLevelEnum.fromString(dispData
				.getEmsAuditLevel());
		AuditLevelEnumProp emsAuditLevelProp = new AuditLevelEnumProp();
		emsAuditLevelProp.setValue(emsAuditLevel);

		log.debug("Audit level event management service: "
				+ dispData.getEmsAuditLevel());

		// --- job service

		AuditLevelEnum jsAuditLevel = AuditLevelEnum.fromString(dispData
				.getJsAuditLevel());
		...
		AuditLevelEnum msAuditLevel = AuditLevelEnum.fromString(dispData
				.getMsAuditLevel());
		...
		AuditLevelEnum psAuditLevel = AuditLevelEnum.fromString(dispData
				.getPsAuditLevel());
		AuditLevelEnumProp psAuditLevelProp = new AuditLevelEnumProp();
		psAuditLevelProp.setValue(psAuditLevel);

		log.debug("Audit level presentation service: "
				+ dispData.getPsAuditLevel());

		// --- report service

		AuditLevelEnum rsAuditLevel = AuditLevelEnum.fromString(dispData
				.getRsAuditLevel());
		...
		AuditLevelEnum ssAuditLevel = AuditLevelEnum.fromString(dispData
				.getSsAuditLevel());
		...
		AuditLevelEnum dispatcherAuditLevel = AuditLevelEnum
				.fromString(dispData.getDispatcherAuditLevel());
		AuditLevelEnumProp dispatcherAuditLevelProp = new AuditLevelEnumProp();
		dispatcherAuditLevelProp.setValue(dispatcherAuditLevel);

		log.debug("Audit level dispatcher: "
				+ dispData.getDispatcherAuditLevel());
		...
		log.debug("Allow native query auditing for report service: "
				+ dispData.isRsNativeQueryAudit());
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setAuditLevels(Dispatcher_Type disp, DispatcherData dispData)
			throws CoCoMaC8Exception {
	 ...
	 * @param dispData
	 */
	private void setCMServiceCustomParameters(Configuration conf,
			DispatcherData dispData) throws CoCoMaC8Exception {

		log.debug("Setting advanced contentManagerService parameters");

		String cmServiceQuery = conf.getSearchPath().getValue()
				+ "//contentManagerService";
		...
		try {
			BaseClass[] cmServices = cmService.query(
					new SearchPathMultipleObject(cmServiceQuery), props, sort,
					queryOptions);

			if (cmServices.length == 0) {
				throw new CoCoMaC8Exception(
						"No ContentManagerService found for the dispatcher named "
								+ this.dispatcherData.getDispatcherName());
				...
				for (BaseClass bcCMService : cmServices) {
					...
					log.debug("ContentManagerService at "
							+ bcCMService.getSearchPath().getValue());
					...
					String customPropertyXML = buildAdvancedPropertyXML(
							"CM.DeploymentIncludeConfiguration", "true");

					log.debug("Advanced content manager service parameters: "
							+ customPropertyXML);
					...
					cmService.update(new BaseClass[] { cms },
							new UpdateOptions());
			...
			throw new CoCoMaC8Exception(
					"Error accessing contentManagerService for dispatcher "
							+ this.dispatcherData.getDispatcherName()
							+ ". C8 Server returned: " + e.getMessage());
		}
	 ...
	 * @param dispData
	 */
	private void setCMServiceCustomParameters(Dispatcher_Type disp,
			DispatcherData dispData) throws CoCoMaC8Exception {

		log.debug("Setting advanced contentManagerService parameters");

		String cmServiceQuery = disp.getSearchPath().getValue()
				+ "/contentManagerService";
		...
		try {
			...
			if (cmServices.length == 0) {
				...
				throw new CoCoMaC8Exception(
						"Dispatcher named "
								+ this.dispatcherData.getDispatcherName()
								+ " has more than one contentManagerService assigned. Returned "
								+ cmServices.length + " results.");
			} else {
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setReportingQueueTimeout(Dispatcher_Type disp,
			DispatcherData dispData) throws CoCoMaC8Exception {

		String queueTimeoutReportingString = dispData
				.getQueueTimeoutReporting();

		log.debug("Setting queue limit for reporting service to "
				+ queueTimeoutReportingString);
		...
		try {
			...
			throw new CoCoMaC8Exception("Queue limit "
					+ queueTimeoutReportingString
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setReportingQueueTimeout(Configuration disp,
			DispatcherData dispData) throws CoCoMaC8Exception {
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setInteractiveReportingProcessLimit(Dispatcher_Type disp,
			DispatcherData dispData) throws CoCoMaC8Exception {
		int maxProcessesCount;

		String maxProcessesCountString = dispData
				.getMaxProcessesInteractiveReporting();

		log.debug("Setting maximum number of processes for report service to "
				+ maxProcessesCountString);

		try {
			maxProcessesCount = Integer.parseInt(maxProcessesCountString);
		} catch (NumberFormatException nfe) {
			throw new CoCoMaC8Exception("Number of high affinity connections "
					+ maxProcessesCountString
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setInteractiveReportingProcessLimit(Configuration conf,
			DispatcherData dispData) throws CoCoMaC8Exception {
		int maxProcessesCount;
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setLowAffintyConnections(Dispatcher_Type disp,
			DispatcherData dispData) throws CoCoMaC8Exception {
		int lowAffinityCount;

		String lowAffinityCountString = dispData.getConnectionsLowAffinity();

		log.debug("Setting low affintity connection count for report service to "
				+ lowAffinityCountString);

		try {
			lowAffinityCount = Integer.parseInt(lowAffinityCountString);
		} catch (NumberFormatException nfe) {
			throw new CoCoMaC8Exception("Number of low affinity connections "
					+ lowAffinityCountString
					+ " cannot be converted to integer.");
		}
		...
		disp.setRsPeakNonAffineConnections(lowAffinityCountProp);
	}
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setLowAffintyConnections(Configuration conf,
			DispatcherData dispData) throws CoCoMaC8Exception {
		int lowAffinityCount;
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setHighAffintyConnections(Dispatcher_Type disp,
			DispatcherData dispData) throws CoCoMaC8Exception {
		...
		log.debug("Setting high affintity connection count for report service to "
				+ highAffinityCountString);
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setHighAffintyConnections(Configuration conf,
			DispatcherData dispData) throws CoCoMaC8Exception {
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setPeakEndTime(Dispatcher_Type disp, DispatcherData dispData)
			throws CoCoMaC8Exception {
		...
		try {
			...
			throw new CoCoMaC8Exception("Peak end hour " + peakEndString
					+ " cannot be converted to integer.");
		}
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setPeakEndTime(Configuration conf, DispatcherData dispData)
			throws CoCoMaC8Exception {
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setPeakStartTime(Dispatcher_Type disp, DispatcherData dispData)
			throws CoCoMaC8Exception {
		...
		try {
			...
			throw new CoCoMaC8Exception("Peak start hour " + peakStartString
					+ " cannot be converted to integer.");
		}
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setPeakStartTime(Configuration conf, DispatcherData dispData)
			throws CoCoMaC8Exception {
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setGovernorLimit(Dispatcher_Type disp, DispatcherData dispData)
			throws CoCoMaC8Exception {
		...
		try {
			...
			throw new CoCoMaC8Exception("Governor limit " + governorLimitString
					+ " cannot be converted to integer.");
		}
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setGovernorLimit(Configuration conf, DispatcherData dispData)
			throws CoCoMaC8Exception {
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setLoadBalacingMode(Dispatcher_Type disp,
			DispatcherData dispData) throws CoCoMaC8Exception {
		...
		if (lbMode.equals(LoadBalancingModeEnum._weightedRoundRobin)) {
			...
			throw new CoCoMaC8Exception("Invalid value for loadBalacingMode: "
					+ lbMode + ". Possible values are: '"
					+ LoadBalancingModeEnum._clusterCompatible + "' or '"
					+ LoadBalancingModeEnum._weightedRoundRobin + "'.");
		}
	 ...
	 * @throws CoCoMaC8Exception
	 */
	private void setLoadBalacingMode(Configuration conf, DispatcherData dispData)
			throws CoCoMaC8Exception {
