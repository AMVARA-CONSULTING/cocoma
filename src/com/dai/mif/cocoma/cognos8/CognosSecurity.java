...
public class CognosSecurity {
    ...
    public void applyCognosSecurity() throws CoCoMaC8Exception {
        ...
        List<AbstractSecurityObject> fixedObjects = this.securityData
                .getFixedObjects();
        purgeCognosSecurity(fixedObjects);

        // create new Cognos security objects
        List<AbstractSecurityObject> securityObjects = this.securityData
                .getSecurityObjects();
    ...
    public void applyContentSecurity() throws CoCoMaC8Exception {
        ...
        List<SecurityPermission> permissions = this.securityData
    ...
    private void setContentPermissions(List<SecurityPermission> permissions) {
        ...
        for (SecurityPermission permission : permissions) {
            ...
            SearchPathMultipleObject spmo = new SearchPathMultipleObject(
                    permission.getTargetName());
            PropEnum[] props = new PropEnum[] { PropEnum.policies,
            ...
            try {
                ...
                if (bcs.length == 1) {
                    ...
                    for (String roleName : permission.getRoleMembers()) {
                        ...
                        if (role != null) {
                            Policy policy = c8Utility.createPolicy(role,
                                    c8Permissions);
                            policies.add(policy);
                        } else {
                    ...
                    for (String groupName : permission.getGroupMembers()) {
                        ...
                        if (group != null) {
                            Policy policy = c8Utility.createPolicy(group,
                                    c8Permissions);
                            policies.add(policy);
                        } else {
                    ...
                    for (String userName : permission.getUserMembers()) {
                        ...
                        if (account != null) {
                            Policy policy = c8Utility.createPolicy(account,
                                    c8Permissions);
                            policies.add(policy);
                        } else {
                    ...
                    if (targetCache.contains(targetPath)) {
                        Policy[] existingPolicies = target.getPolicies()
                                .getValue();
                    ...
                    String msg = "Permission target "
                            + permission.getTargetName() + " is ambiguous";
                    CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_MINOR_ERROR, msg);
                    log.error(msg);
                } else {
                    String msg = "Permission target "
                            + permission.getTargetName() + " does not exist";
                    CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_MINOR_ERROR, msg);
                    log.error(msg);
                }
            } catch (RemoteException e) {
                String msg = "Error setting permissions on "
                        + permission.getTargetName() + ": " + e.getMessage();
     ...
     */
    private void extendCognosSecurityObjects(
            List<AbstractSecurityObject> securityObjects) {
        ContentManagerService_PortType cms = this.c8Access.getCmService();
        ...
        try {
            ...
            for (AbstractSecurityObject aso : securityObjects) {
                ...
                if (bc != null) {
                    ...
                    String parent = "CAMID(\":\")";
                    query = "//" + bc.getClass().getSimpleName().toLowerCase()
                            + "[@name='" + aso.getName() + "']";
                    ...
                    membersProp.setProperties(new PropEnum[] {
                            PropEnum.defaultName, PropEnum.userName,
                            PropEnum.searchPath });
                    ...
                    String checkTarget = bc.getClass().getSimpleName() + " '"
                            + aso.getName() + "'";
                    log.debug("Checking if " + checkTarget + " already exists.");

                    BaseClass[] bcs = cms.query(spmo, props, new Sort[] {},
                            queryOptions);
                    ...
                    if (bcs.length < 1) {
                        ...
                        log.debug(checkTarget + " is ambiguous. " + bcs.length
                                + " results.");
                    } else {
                    ...
                    if (!targetExists) {
                        log.debug("Adding " + bc.getClass().getSimpleName()
                                + " " + aso.getName() + " to Cognos namespace");
                    ...
                    String msg = "Could not create Cognos object for "
                            + aso.getClass().getSimpleName() + " "
                            + aso.getName();
                    CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_CRTICAL_ERROR, msg);
                    log.error(msg);
                }
            ...
            for (AbstractSecurityObject aso : securityObjects) {
                ...
                if (aso instanceof RoleSecurity) {
                    ...
                    if (!accounts.isEmpty()) {
                        rs.addSecurityNamespaceMembers(accounts
                                .toArray(new Account[] {}));
                    }
                    ...
                    if (!accounts.isEmpty()) {
                        gs.addSecurityNamespaceMembers(accounts
                                .toArray(new Account[] {}));
                    }
                    if (log.isDebugEnabled()) {
                        gs.dumpMembers();
            ...
            BaseClass[] bcs = cms.update(
                    modifiedSecurityObjects.toArray(new BaseClass[] {}),
                    updateOptions);
            if (bcs.length != modifiedSecurityObjects.size()) {
                String msg = "Error updating extended security objects. "
                        + bcs.length + " of " + modifiedSecurityObjects.size()
                        + " have been updated.";
                CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_CRTICAL_ERROR, msg);
                log.error(msg);
            } else {
     ...
     */
    private void purgeCognosSecurity(List<AbstractSecurityObject> fixedObjects)
        ...
        try {
            ...
            for (BaseClass bc : results) {
                log.debug(bc.getClass().getSimpleName() + " "
            ...
            log.info(fixedCount + " security objects will be kept ("
                    + deactivedCount + " deactived)");

            // if there have been changes to the existing objects, update them
            if (!updatedObjects.isEmpty()) {

                log.info("Updating modified security objects ("
                        + updatedObjects.size() + ")");

                UpdateOptions updateOptions = new UpdateOptions();
                cms.update(updatedObjects.toArray(new BaseClass[] {}),
                        updateOptions);
            ...
            if (!deletedObjects.isEmpty()) {
                ...
                log.info("Deleting unwanted security objects ("
                        + deletedObjects.size() + ")");

                DeleteOptions deleteOptions = new DeleteOptions();
                deleteOptions.setRecursive(true);
                cms.delete(deletedObjects.toArray(new BaseClass[] {}),
                        deleteOptions);
