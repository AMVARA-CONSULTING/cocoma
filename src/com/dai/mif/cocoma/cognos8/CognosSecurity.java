...
public class CognosSecurity {
	...
	private void setContentPermissions(List<SecurityPermission> permissions) {
		...
		List<String> targetCache = new ArrayList<>();
		...
		for (SecurityPermission permission : permissions) {
			...
			// prepare the query
			log.debug("Target name from config permission: "+permission.getTargetName());
			...
			PropEnum[] props = new PropEnum[] { 
					...
					PropEnum.searchPath,
					PropEnum.effectiveUserCapabilities, 
			...
			try {
				...
				if (bcs.length == 1) {
					...
					log.debug("Target from baseclass working on: "+targetPath);
					...
					// Get UserCapabilityPolicy
					ArrayList<UserCapabilityPolicy> userCapabilityPolicies = new ArrayList<>();
					// loop over all the user capability policies from permissions
					for (Entry<HashMap<String, String>, UserCapabilityPermission[]> userCapabilityPolicy : permission
							.getUserCapabilitiesPolicies().entrySet()) {
						// get tag key from HashMap
						Map.Entry<String, String> entry = userCapabilityPolicy.getKey().entrySet().iterator().next();
						String tagKey = entry.getKey();
						String securityObjectName = entry.getValue();

						switch (tagKey) {
						case "role":
							// find role by role name
							Role role = c8Utility.findRole(securityObjectName);
							if (role != null) {
								UserCapabilityPolicy userCapPolicy = new UserCapabilityPolicy();
								userCapPolicy.setSecurityObject(role);
								userCapPolicy.setPermissions(userCapabilityPolicy.getValue());
								userCapabilityPolicies.add(userCapPolicy);
							} else {
								String msg = "Role \"" + securityObjectName + "\" could not be found.";
								CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_MINOR_ERROR, msg);
								log.error(msg);
							}
							break;
						case "group":
							...
							Group group = c8Utility.findGroup(securityObjectName);
							if (group != null) {
								UserCapabilityPolicy userCapPolicy = new UserCapabilityPolicy();
								userCapPolicy.setSecurityObject(group);
								userCapPolicy.setPermissions(userCapabilityPolicy.getValue());
								userCapabilityPolicies.add(userCapPolicy);
							} else {
								String msg = "Group \"" + securityObjectName + "\" could not be found.";
								CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_MINOR_ERROR, msg);
								log.error(msg);
							}
							break;
						case "user":
							...
							Account account = c8Utility.findAccount(securityObjectName);
							if (account != null) {
								UserCapabilityPolicy userCapPolicy = new UserCapabilityPolicy();
								userCapPolicy.setSecurityObject(account);
								userCapPolicy.setPermissions(userCapabilityPolicy.getValue());
								userCapabilityPolicies.add(userCapPolicy);
							} else {
								String msg = "User \"" + securityObjectName + "\" could not be found.";
								CoCoMa.setErrorCode(CoCoMa.COCOMA_ERROR_MINOR_ERROR, msg);
								log.error(msg);
							}
							break;
					...
					/**
					 * =========================================================== Append current
					 * UserCapabilityPolicies to the arraylist Commented because we are replacing
					 * existing values. Uncomment the code if needed in future.
					 * =========================================================== if
					 * (target.getClass().getName().contains("Content")) { userCapabilityPolicies
					 * .addAll(Arrays.asList(((Content)
					 * target).getUserCapabilityPolicies().getValue())); } else if
					 * (target.getClass().getName().contains("_package")) { userCapabilityPolicies
					 * .addAll(Arrays.asList(((_package)
					 * target).getUserCapabilityPolicies().getValue())); } else if
					 * (target.getClass().getName().contains("Folder")) { userCapabilityPolicies
					 * .addAll(Arrays.asList(((Folder)
					 * target).getUserCapabilityPolicies().getValue())); }
					 */

					// check if there are any policies configured if not leave existing ones be.
					if (userCapabilityPolicies.size() > 0) {
						// create a UserCapabilityPolicyArrayProp
						UserCapabilityPolicyArrayProp userCapabilityPoliciesArrayProp = new UserCapabilityPolicyArrayProp();
						userCapabilityPoliciesArrayProp
								.setValue(userCapabilityPolicies.toArray(new UserCapabilityPolicy[0]));

						// set the userCapabilityPolicies
						if (target.getClass().getName().contains("Content")) {
							((Content) target).setUserCapabilityPolicies(userCapabilityPoliciesArrayProp);
						} else if (target.getClass().getName().contains("_package")) {
							((_package) target).setUserCapabilityPolicies(userCapabilityPoliciesArrayProp);
						} else if (target.getClass().getName().contains("Folder")) {
							((Folder) target).setUserCapabilityPolicies(userCapabilityPoliciesArrayProp);
						}
					}
	...
	private void extendCognosSecurityObjects(List<AbstractSecurityObject> securityObjects) {
		...
		try {
			...
			ArrayList<BaseClass> modifiedSecurityObjects = new ArrayList<>();

			log.debug("Extending new security objects.");
			log.debug("... this will set the members of the Security Objects");
			...
			log.debug("Updating extended security objects with values from gathered in memory .");
	...
	private void purgeCognosSecurity(List<AbstractSecurityObject> fixedObjects) throws CoCoMaC8Exception {
		...
		List<BaseClass> updatedObjects = new ArrayList<>();
		List<BaseClass> deletedObjects = new ArrayList<>();
