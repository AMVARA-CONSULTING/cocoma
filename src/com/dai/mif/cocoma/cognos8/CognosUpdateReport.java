...
public class CognosUpdateReport {
	...
	public void updateQueryReport(C8Access c8Access, String reportSearchPath, String packageSearchPath) throws Exception
	{
			...
			PropEnum props[] = new PropEnum[]{PropEnum.searchPath,PropEnum.metadataModel,PropEnum.metadataModelPackage};
			...
			BaseClass[] report = this.cmService.query(searchPath,props, new Sort[]{}, new QueryOptions());
			

			if( report.length!=0) {
				log.info("Reports found: " + report.length);
				log.info("each report update may take one or more seconds.");
				if (report.length>50) log.info("please be patient. Set loglevel to DEBUG to see details.");
				for(int i = 0; i < report.length; i++)
				{
					try {
						if (report[i] instanceof Query || report[i] instanceof Report ){
							//get the search path of the package/model associated to this report or query
							String pkgPath = new String();
							
							if (report[i] instanceof Report)
								{
								  log.debug("Report ("+i+"/"+report.length+")");
								  pkgPath=((Report)report[i]).getMetadataModelPackage().getValue()[0].getSearchPath().getValue().toString();
								}
							else
							{
								  log.debug("Query ("+i+"/"+report.length+")");
								  pkgPath=((Query)report[i]).getMetadataModelPackage().getValue()[0].getSearchPath().getValue().toString();
								
							}
							log.debug("Packagepath: "+pkgPath);
							if ( packageSearchPath==null || packageSearchPath.length()==0 || fromPackage(pkgPath,packageSearchPath)){
								log.debug("Found upgradeable Report|Query");

								log.debug("Will set Package to "+pkgPath+"/model[last()]");
								...
								{
									((Query)report[i]).setMetadataModel(bcArrProp2);
								}
								else
									((Report)report[i]).setMetadataModel(bcArrProp2); 							 
								log.debug("MetadataModell was set to last()");
								
								// 
								log.debug("Upgradeing Report|Query");
								cmService.update(new BaseClass[]{report[i]},new UpdateOptions());
								log.debug( i+"/"+report.length+" has been updated: "+report[i].getSearchPath().getValue() );
							} else {
								log.debug("nothing todo");
							}
						}
						else
							log.info(report[i].getSearchPath().getValue() + " is not a report or query.");
					}
					catch (Exception ex)
					{
						log.error("!!! Exception found");
						log.error("Unable to update object: " + report[i].getSearchPath().getValue());
					}
				} 
			}
			else 
	...
	public static boolean fromPackage(String rptPackage,String comparePackagePath)
	{
		//determine if the report or query was created against the republished package
